...
The Tetris Inventory builds on the same fragment architecture used across the entire Lyra item system. Fragments like `InventoryFragment_ItemDetails` (name, weight, stacking), `InventoryFragment_Icon` (display icon), and `InventoryFragment_SetStats` (stat tags) work identically here, every Tetris item will use these.
...
│   ─────────────────────             ─────────────────────────          │
│   InventoryFragment_ItemDetails     InventoryFragment_Tetris           │
│   InventoryFragment_Icon            InventoryFragment_Container        │
│   InventoryFragment_SetStats        InventoryFragment_CraftRecipe      │
│   InventoryFragment_Equippable                                         │
│                                                                        │
└────────────────────────────────────────────────────────────────────────┘
```
...
**Weapon with inspection**: `Tetris` (1x3 shape) + the base [Inspect Fragment](../../../base-lyra-modified/items/item-fragments-in-depth/inspect-fragment.md) (3D model, camera limits) + base fragments.

...
Before scheduling the deferred focus, the Layer stores the navigation direction and cursor screen position:
...
When the target window's shell receives focus on the next frame, it consumes the pending context:
...
{% step %}
**Handle focus restoration gracefully**

If your content is stateful (like a tetris grid with a cursor), restoring focus should just call `SetFocus()` on the appropriate widget without repositioning the cursor. The `bIsRestoringFocus` flag handles this at the shell level — your content does not need to worry about it unless you override `RequestContentFocus`.
{% endstep %}
{% endstepper %}

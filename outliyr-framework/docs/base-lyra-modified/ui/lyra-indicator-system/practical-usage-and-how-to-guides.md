...
1. **Design the UMG Widget (Visual Representation):**
   * Create a new UMG `UserWidget` Blueprint (e.g., `WBP_QuestIndicator`).
   * **Implement the Interface:** In the "Class Settings" of your new UMG widget, under the "Interfaces" section, add the `IActorIndicatorWidget` interface.
   ...
   * **Implement Interface Functions:**
     *   **`Event Bind Indicator`:**
         ...
         * **Access Data Object (Optional):** If you plan to use the `DataObject` on the descriptor, get it, cast it to your expected custom data type (e.g., `BPI_QuestDataInterface`), and store it as well.
         * **Initialize Visuals:** Use the data from the `Indicator Descriptor` (or its `DataObject`) to set up your widget's initial appearance. For example, set the icon image, bind text to a property from the `DataObject`, etc.
         ...
         ```
         // Example in WBP_QuestIndicator's Event Bind Indicator
         Set QuestIndicatorDescriptor (From Event Parameter)

         // Optional: If using DataObject
         QuestData = QuestIndicatorDescriptor -> GetDataObject -> Cast To BPI_QuestDataInterface
         if (IsValid(QuestData)) {
             MyTextBlock -> SetText (QuestData -> GetQuestName)
         }
         ```
     ...
     *   **`Event On Indicator Clamped`:**
         ...
         * You can change the widget's appearance based on this. For example, hide certain details or show a different icon when clamped.
         ...
         ```
         // Example in WBP_QuestIndicator's Event On Indicator Clamped
         if (bIsClamped) {
             MyClampedIcon -> SetVisibility (Visible)
             MyDefaultIcon -> SetVisibility (Collapsed)
         } else {
             MyClampedIcon -> SetVisibility (Collapsed)
             MyDefaultIcon -> SetVisibility (Visible)
         }
         ```
...
2. **Define How to Spawn and Register the Indicator (C++ or Blueprint):**
   ...
   * **Create and Configure the `UIndicatorDescriptor`:**
     * **Create Instance:** Spawn a new `UIndicatorDescriptor` object. In C++, use `NewObject<UIndicatorDescriptor>(ManagerComponent)`. In Blueprint, you can use the `Construct Object from Class` node, ensuring its "Outer" is set to the `IndicatorManagerComponent` (or another persistent object if the manager isn't available yet, though the manager is a good default outer).
     * **Set Target:**
   ...
   *   **Example Blueprint Snippet (e.g., in a Quest Manager Actor when a quest becomes active):**

       ```blueprint
       // Get Player Controller (e.g., GetPlayerController index 0)
       PlayerController = ... 

       // Get Indicator Manager
       IndicatorManager = GetIndicatorManagerComponent (Controller: PlayerController)

       if (IsValid(IndicatorManager) AND IsValid(QuestObjectiveActor)) {
           // Create Descriptor
           NewDescriptor = Construct Object from Class (Class: UIndicatorDescriptor, Outer: IndicatorManager)
           
           // Configure Descriptor
           NewDescriptor -> Set Scene Component (Target: QuestObjectiveActor -> GetRootComponent)
           NewDescriptor -> Set Indicator Class (Class: WBP_QuestIndicator)
           NewDescriptor -> Set Projection Mode (ProjectionMode: ActorBoundingBox)
           NewDescriptor -> Set Clamp To Screen (bValue: true)
           NewDescriptor -> Set Show Clamp To Screen Arrow (bValue: true)
           NewDescriptor -> Set HAlign (HAlignment: Center)
           NewDescriptor -> Set VAlign (VAlignment: Center)
           // ... other settings ...

           // Optional: Set Data Object
           // QuestDataObject = Construct Object from Class (Class: MyQuestDataObject_BP, Outer: NewDescriptor)
           // QuestDataObject -> InitializeQuestDetails(...)
           // NewDescriptor -> Set Data Object (DataObject: QuestDataObject)
           
           // Add to Manager
           IndicatorManager -> Add Indicator (IndicatorDescriptor: NewDescriptor)

           // Store NewDescriptor if you need to remove/modify it later
           ActiveQuestIndicator = NewDescriptor 
       }
       ```
...
* **Dynamic Objective Indicator (e.g., Domination Control Point):**
  ...
  * **Initial Setup (Objective in the World):**
    ...
    * **Widget Class:** A UMG widget designed to show objective status (e.g., WBP\_ControlPointIndicator showing capture progress, faction icon).
    * **Projection Mode:** ComponentPoint or ActorBoundingBox.
  ...
  * **Player Enters Control Point Zone (Transition to 2D Screen-Locked):**
    * When game logic detects the player has entered the control point's capture area (e.g., via an overlap volume):
      ...
      3. Call SwitchTo2DMode() on the descriptor. This will:
         ...
         * Notify the bound UMG widget via IActorIndicatorWidget::OnIndicatorDisplayModeChanged(true).
    * The UMG widget (WBP\_ControlPointIndicator) can then react to OnIndicatorDisplayModeChanged(true) by:
  ...
  * **Player Exits Control Point Zone (Transition back to 3D World-Tracking):**
    * When game logic detects the player has left the control point's capture area:
      ...
      2. Call SwitchTo3DMode() on the descriptor. This will:
         ...
         * Notify the bound UMG widget via IActorIndicatorWidget::OnIndicatorDisplayModeChanged(false).
